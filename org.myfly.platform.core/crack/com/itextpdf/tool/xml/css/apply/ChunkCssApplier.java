/*
 * $Id: ChunkCssApplier.java 393 2013-03-13 08:36:21Z pavel-alay $
 *
...
public class ChunkCssApplier {
	/**
	 * FF4 and IE8 provide normal text and bold text. All other values are
	 * translated to one of these 2 styles <br />
	...
	public Chunk apply(final Chunk c, final Tag t) {
		...
		if (null != bfCN && isChinese(c.getContent())) {
			f = new Font(bfCN, f.getSize(), f.getStyle(), f.getColor());
		}
		...
		for (Entry<String, String> entry : rules.entrySet()) {
			...
			} else if (null != rules.get(CSS.Property.XFA_FONT_HORIZONTAL_SCALE)) { // only
																					// %
																					// allowed;
																					// need
																					// a
		...
		// following styles are separate from the for each loop, because they
		// are based on font settings like size.
		...
		if (null != rules.get(CSS.Property.TEXT_DECORATION)) { // Restriction?
																// In html a
																// underline and
																// a
																// line-through
	...
	public Font applyFontStyles(final Tag t) {
		...
		for (Entry<String, String> entry : rules.entrySet()) {
			...
			if (CSS.Property.FONT_WEIGHT.equalsIgnoreCase(key)) {
				...
				// TODO improve fontfamily parsing (what if a font family has a
				// comma in the name ? )
				fontName = value;
	...
	/**
	 * Method used for retrieving the widest word of a chunk of text. All styles
	 * of the chunk will be taken into account when calculating the width of the
	 * words.
	 *
	 * @param c
	...
	/**
	 * Method used for copying styles from one chunk to another. Could be
	 * deprecated if the content of a chunk can be overwritten.
	 *
	 * @param source
