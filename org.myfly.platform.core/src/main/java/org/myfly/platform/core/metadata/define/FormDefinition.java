...
public class FormDefinition extends BaseDenifition {
	...
	public void setSectionsFromView(SectionView[] views) {
		sections = Stream.of(views).map(item -> new SectionDefinition(item)).collect(Collectors.toList())
	 ...
	 * @return
	 */
	// public static FormDefinition buildFormDefinition(EntityMetaData metaData,
	// FormView formView) {
	// FormDefinition formDefinition = new FormDefinition(metaData,
	// formView.name());
	...
	// int index = 0;
	// for (SectionView sectionView : formView.sections()) {
	// SectionDefinition sectionDefinition = new SectionDefinition(formDefinition,
	// sectionView.title());
	// if (StringUtils.isBlank(sectionView.name())) {
	...
	// CommonSubTableType commonSubTableType =
	// CommonSubTableType.valueOf(sectionView.type().name());
	...
	// }
	// } else {
	// formDefinition = buildDefaultFormViewDefinition(metaData,
	...
	public static FormDefinition buildDefaultFormViewDefinition(EntityMetaData metaData, String name) {
		...
		/*
		 * List<SectionDefinition> sections = new ArrayList<>(); //
		 * 主记录所有属性建立一个Section和二个FieldSet SectionDefinition sectionDefinition = new
		 * SectionDefinition(formDefinition, "基本信息"); FieldSetDefinition
		 * fieldSetDefinition1 = new FieldSetDefinition("基本信息一"); FieldDefinition[]
		 * fieldSet1 = ArrayUtils.subarray(metaData.getAllFields(), 0,
		 * metaData.getAllFields().length / 2);
		 * fieldSetDefinition1.setFields(fieldSet1); FieldSetDefinition
		 * fieldSetDefinition2 = new FieldSetDefinition("基本信息二"); FieldDefinition[]
		 * fieldSet2 = ArrayUtils.subarray(metaData.getAllFields(),
		 * metaData.getAllFields().length / 2, metaData.getAllFields().length);
		 * fieldSetDefinition2.setFields(fieldSet2); sectionDefinition.setFieldSets(new
		 * FieldSetDefinition[] { fieldSetDefinition1, fieldSetDefinition2 });
		 * sections.add(sectionDefinition);
		 * 
		 * // 每一个子表建立一个Section FieldDefinition[] subTableFields = metaData
		 * .getFieldsByDataType(new FieldDataType[] { FieldDataType.MDRELATION,
		 * FieldDataType.FLYMDRELATION }); for (FieldDefinition subTableField :
		 * subTableFields) { SubTableDefinition subTable =
		 * SubTableDefinition.buildSubTable(metaData, subTableField,
		 * EntityMetaDataConstants.DEFAULT_NAME); if (subTable != null) {
		 * subTable.setName(formDefinition.getName());
		 * 
		 * sectionDefinition = new SectionDefinition(formDefinition,
		 * subTable.getTitle()); sectionDefinition.addSubTable(subTable);
		 * sections.add(sectionDefinition); } }
		 * formDefinition.setSections(sections.toArray(new SectionDefinition[] {}));
		 * 
		 * // 没有定义布局时，生成默认布局 if (ArrayUtils.isNotEmpty(formDefinition.getSections())) {
		 * // 默认布局： <div width=8> </div> <div width=4> <div name='' class='SOCIAL'>
		 * </div> DivDefinition[] divs = new DivDefinition[2]; divs[0] = new
		 * DivDefinition(); divs[0].setWidth(8); int index = 0; DivDefinition[] div1s =
		 * new DivDefinition[formDefinition.getSections().length]; for
		 * (SectionDefinition sectionDefinition2 : formDefinition.getSections()) {
		 * sectionDefinition2.setName("div" + index); DivDefinition divDefinition = new
		 * DivDefinition(); divDefinition.setName(sectionDefinition2.getName());
		 * div1s[index] = divDefinition; index++; } divs[0].setSubs(div1s);
		 * 
		 * // 增加特性Section SectionDefinition socialSection = new
		 * SectionDefinition(formDefinition, "活动"); socialSection.setName("div" +
		 * sections.size()); socialSection.setType(SectionType.SOCIAL);
		 * 
		 * divs[1] = new DivDefinition(); divs[1].setWidth(4);
		 * divs[1].setName(socialSection.getName());
		 * 
		 * formDefinition.setSections(ArrayUtils.add(formDefinition.getSections(),
		 * socialSection)); formDefinition.setDivs(divs); }
		 */
		return formDefinition;
	}
	...
	public void validate() {
		...
		if (ArrayUtils.isNotEmpty(getDivs())) {
			Stream.of(getDivs()).forEach(item -> {
				item.validate();
			});
		}
	}
