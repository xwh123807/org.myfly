                        ...
                        <el-row>
                            <el-col :span="column.dataType === 'Text' ? 3 : 6" style="text-align:right">
                                <span v-if="column.dataType === 'Table' || column.dataType === 'TableDirect' || column.dataType === 'List'">
                                    <a href="#" @click="viewRelationFieldHandler" :m="column.relationTable">{{column.name}}&nbsp;&nbsp;</a>
                                </span>
                            ...
                            <el-col :span="column.dataType === 'Text' ? 21 : 18">
                                ...
                                <el-select v-else-if="column.dataType === 'List'" v-model="data[column.model]" style="width:100%" clearable>
                                    <el-option v-for="item in column.refListItems" v-bind:key="item[column.relationKeyColumn]" 
                                        :value="item[column.relationKeyColumn]" :label="item[column.relationDisplayColumn]" @change="selectChangeHandler(column, data)"></el-option>
                                </el-select>
...
export default {
  ...
  methods: {
    ...
    viewRelationFieldHandler(column, data) {
      var route = {
        name: "dynamicFormWindow",
        params: {
          windowName: column.relationTable,
          uid: data[column.columnName]
            ? data[column.columnName][column.relationKeyColumn]
            : ""
        }
      };
      this.$router.push(route);
    },
    selectChangeHandler(p1) {
      console.info(p1);
    }
  }
};
